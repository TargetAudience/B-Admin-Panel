...
if(!defined('TMVC_MYAPPDIR'))
  define('TMVC_MYAPPDIR', TMVC_BASEDIR . 'portalapp' . DS);
...
set_include_path(get_include_path()
  ...
  . PATH_SEPARATOR . TMVC_MYAPPDIR . 'libs' . DS
...
class tmvc
{
  ...
  public function main()
  {
    ...
    $currentControllerName = !empty($this->url_segments[1]) ? preg_replace('!\W!','',$this->url_segments[1]) : '';
    // To avoid too many redirects
    if ($currentControllerName != 'accessDenied') {
        $this->checkAuthorization($currentControllerName);
    }
  ...
  public function checkAuthorization($currentControllerName) {
    $this->controller->load->model('Administration_Model', 'administration');

    // $adminControllers will be in format [controllerName => roleId]
    $adminControllers = [];
    foreach($_SESSION['adminRoles'] as $role) {
        $adminControllers[$role['controllerName']] = $role['id'];
    }

     // Validate controller only if it exists in adminroles table
    if (array_key_exists($currentControllerName, $adminControllers)) {
        $roleId = $adminControllers[$currentControllerName];
        //  Checking role required to access current page
        if (!array_key_exists($roleId, $_SESSION['role'])) {
            header("Location: " . TMVC_URL . "accessDenied");
            exit();
        }
    }
  }
