...
class GUMP
{
		...
		foreach($this->errors as $e) {
			...
			switch($e['rule']) {
				...
				case 'validate_zip_code':
					$resp[] = "The <span class=\"$field_class\">$field</span> field may only contain numeric characters &amp; dashes";
					break;
		...
		foreach($this->errors as $e) {
			...
			switch($e['rule']) {
				...
				case 'validate_alpha_dash':
					...
					break;
				case 'validate_zip_code':
					$resp[$field] = "The $field field may only contain numeric characters &amp; dashes";
					break;
				case 'validate_numeric':
	...
	*/
	 ...
	 * @return mixed
	 */
	protected function validate_zip_code($field, $input, $param = NULL)
	{
		if(!isset($input[$field]) || empty($input[$field]))
		{
			return;
		...
		if(!preg_match("/^([0-9ÀÁÂÃÄÅÇÈÉÊËÌÍÎÏÒÓÔÕÖÙÚÛÜÝàáâãäåçèéêëìíîïðòóôõöùúûüýÿ-])+$/i", $input[$field]) !== FALSE)
		{
			return array(
				'field' => $field,
				'value' => $input[$field],
