(function($) {
    ...
    $(document).ready(function() {
        ...
        $('.table-layout tr').click(function(e) {
            if(!$(e.target).is('.box')) {
                $(this).find('.box').click();
            }
        });
        ...
        var optsUpload = {
            url: 'imageUpload?imageType=thumb&id=' + window.imageId + '&type=' + window.type,
            fileHolder: '#file_1',
            ...
            onSuccess: function (server_return, name, uploaded_file) {
                ...
                var _drop = $('#drop1');
                _drop.after($('<div class="message-saved" />').html('Image saved.'));
            },
            onFail: function (server_response) {
                console.log('server_response', server_response)
            }
        };
        $('#drop1').droparea(optsUpload);

        var optsUpload = {
            url: 'imageUpload?imageType=main&id=' + window.imageId + '&type=' + window.type,
            fileHolder: '#file_2',
            filePreview: '#file_preview_2',
            notificationDelay: 120000,
            onSuccess: function (server_return, name, uploaded_file) {
                var _prev = $('#file_preview_2');
                var _drop = $('#drop2');
                _drop.after($('<div class="message-saved" />').html('Image saved.'));
            },
            onFail: function (server_response) {
        ...
        $('#drop2').droparea(optsUpload);

        var optsUpload = {
            url: 'imageUploadMeals?imageType=image&id=' + window.imageMealItemId,
            fileHolder: '#file_3',
            filePreview: '#file_preview_3',
            notificationDelay: 120000,
            onSuccess: function (server_return, name, uploaded_file) {
                var _prev = $('#file_preview_3');
                var _drop = $('#drop3');
                _drop.after($('<div class="message-saved" />').html('Image saved.'));
            },
            onFail: function (server_response) {
        ...
        $('#drop3').droparea(optsUpload);

        var optsUpload = {
            url: 'imageUploadMeals?imageType=thumbnail&id=' + window.imageMealItemId,
            fileHolder: '#file_4',
            filePreview: '#file_preview_4',
            notificationDelay: 120000,
            onSuccess: function (server_return, name, uploaded_file) {
                var _prev = $('#file_preview_4');
                var _drop = $('#drop4');
                _drop.after($('<div class="message-saved" />').html('Image saved.'));
            },
            onFail: function (server_response) {
        ...
        $('#drop4').droparea(optsUpload);

        $('.show-edit').focus(function(e) {
            $(this).css('background', '#d3ebfd');
            e.preventDefault();
        });

        $('.show-edit').blur(function(e) {
            var that = $(this);
            var id = $(this).attr('data-id');
            var type = $(this).attr('data-type');

            $(this).css('background-color', '#fff');
            $(this).css('background', 'url(' + window.domainBase + '/assets/portalapp/images/loaderIcon.gif) no-repeat center right 5px');
            
            $.ajax({
                type: 'POST',
                url: window.domainBase + '/equipment/editInStock',
                data: 'type=' + type + '&value=' + $(this).html() + '&id=' + id,
