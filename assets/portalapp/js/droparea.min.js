...
(function($) {
            ...
            oFReader.onload = function(oFREvent) {
                $(o.filePreview).animate({
                    ...
                    $(o.filePreview).animate({
                        ...
                        drop_area.find('.statusbar.alert-block').fadeOut('slow', function() {
                            $(this).remove();
                        });
                    });
                });
    ...
    var _onClickDismiss = function(ev) {
        ev.stopPropagation();
        ev.preventDefault();
        $(ev.currentTarget).parent().fadeOut('fast', function() {
            $(this).remove();
        });
    };
    var _createStatusBar = function(drop_area) {
        ...
        var setProgress = function(progress) {
            progressBarElem.find('div').animate({
                width: progress + '%'
            }, 10).html(progress + o.i18n.percent);
        ...
        statusBarElem.append(fileNameElem, fileSizeElem, progressBarElem, abortElem);
    ...
    var _createAlertBlock = function(target, msg, dismissable, autohide) {
        ...
        dismiss = $('<button class="btn dismiss"></button>').html(opts.i18n.dismiss);
        alertblock.css({
            top: 0,
            left: 0
        });
        ...
        if (dismissable) {
            dismiss.off('click.droparea').on('click.droparea', _onClickDismiss).appendTo(alertblock);
        }
        if (autohide) {
            setTimeout(function() {
    ...
    var _sendFileToServer = function(drop_area, o, file) {
        ...
        var onXHRSuccess = function(r) {
            ...
            if (o.onSuccess !== null && $.isFunction(o.onSuccess)) {
                var fileNameToReturn = typeof r.file_name !== 'undefined' ? r.file_name : null;
        ...
        if (o.extra !== null) {
            for (var i in o.extra) {
                if ($('#' + o.extra[i].replace('#', '')).length) {
                    ajaxOpts.data.append(o.extra[i].replace('#', ''), $('#' + o.extra[i].replace('#', '')).val());
                }
            }
        }
        statusbar = new _createStatusBar(drop_area);
        statusbar.setFileNameSize(file.name, file.size);
        jqXHR = $.ajax(ajaxOpts).then(onXHRSuccess).fail(onXHRFail).always(onXHRComplete);
        statusbar.setAbort(jqXHR);
    };
    var _interceptFileSelector = function(drop_area, o) {
        if ($(o.fileHolder).length && $(o.filePreview).length) {
            $(o.fileHolder).off('change.droparea').on('change.droparea', (event) => {
                var file;
                if (event.originalEvent && event.originalEvent.target && event.originalEvent.target.files) {
    ...
    var _onDrop = function(ev) {
        ...
        if (o.accepted !== null && typeof o.accepted !== 'boolean') {
            ...
            if (accepting.indexOf(extension) === -1) {
                _createAlertBlock(drop_area, o.i18n.wrongFileType.replace('%s', '<b >' + o.accepted.split('|').join('</b> or <b >') + '</b>'));
                return;
            }
        }
        if (file.size / 1024 > o.fileMaxSize) {
            _createAlertBlock(drop_area, o.i18n.wrongFileSize.replace('%s', '<b >' + o.fileMaxSize + ' ' + o.i18n.kb + '</b>'));
            return;
        }
        _sendFileToServer(drop_area, o, file);
    };
    var _initHandlers = function(drop_area) {
        drop_area.off('click.droparea').on('click.droparea', _onClick).off('dragenter.droparea').on('dragenter.droparea', _onDragEnter).off('dragleave.droparea').on('dragleave.droparea', _onDragLeave).off('dragover.droparea').on('dragover.droparea', _onDragOver).off('drop.droparea').on('drop.droparea', _onDrop);
    };
    var _construct = function(options) {
        return this.each(function() {
            ...
            drop_area.parent().css('position', drop_area.parent().css('position') || 'relative');
